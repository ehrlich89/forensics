...
The idea behind Relution Tags is to assign each beacon a number of tags that the beacon will send out periodically. A mobile app using this SDK scans for these tags and individually interprets them by e.g. executing an action, showing the user product information etc. Since the app itself is responsible for mapping each tag to a specific behavior, there is no need of continuous internet availability. Although the concept of Relution tags does not allow you to change the app's behavior dynamically, it is possible to exchange the tags a beacon sends out in the management platform and, thus, gives your app a partial dynamic behavior without requiring constant internet access. 
