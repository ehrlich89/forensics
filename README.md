...
#### Triggering
- A stream of beacon messages can trigger an action which will be executed based different time and location based parameters. E.g. actions can be exectuted after a predefined delay or locked for a specific time. Moreover, actions can be defined to trigger only inside a certain distance range from the beacon or can have a validation period.
