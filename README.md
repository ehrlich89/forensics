...
The "BlueRange SDK for Android" is a library for the Android platform providing an API to develop applications that interact with Bluetooth Low Energy beacons in general and BlueRange SmartBeacons in specific. The API is intended to provide an easy to use interface to develop time and location based applications that make use of BlueRange mesh networks. For more information about the BlueRange technology, have a closer look at this project on GitHub. The SDK also provides a seamless integration into an environment based on the Enterprise Mobility Management platform Relution. The Relution SmartBeacon Management is part of Relution and allows you to control and monitor a mesh of SmartBeacons. Using Relution you can assign each beacon a message that will send out in regular intervals. At the moment Relution supports iBeacons and Relution Tag messages. The idea behind Relution Tags is to assign each beacon a number of tags that will be scanned by the mobile application and individually interpreted without having the neccessity of continuous internet availability. Although the app's internal behavior may not change, since it is hard coded and not defined by Relution, it is possible to change the tags being sent by a beacon dynamically. iBeacons, on the other side, are currently used for the realization of so called campaigns. A campaign defines a set of actions that will be triggered by a mobile device, whenever the device is next to beacons that take part in the campaign.
