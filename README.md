...
The SDK is specifically designed to interact with beacons that are enrolled in the Relution Beacon Management platform. E.g. you can use the management platform to instruct beacons to send a message which can later trigger actions on a Android or iOS device using this SDK. At the moment the SDK supports iBeacon messages and custom Relution Tag messages. 
