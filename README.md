...
The "BlueRange SDK for Android" is a library for the Android platform providing an API to develop applications that interact with Bluetooth Low Energy beacons in general and BlueRange SmartBeacons in specific. The API is intended to provide an easy to use interface to develop time and location based applications that make use of BlueRange mesh networks. For more information about the BlueRange technology, have a closer look at this project on GitHub. The SDK also provides a seamless integration into an environment based on the Enterprise Mobility Management platform Relution. The Relution SmartBeacon Management is part of Relution and allows you to control and monitor a mesh of SmartBeacons. Before messages and actions can be assigned to beacons, each beacon has to be enrolled by a MeshGateway that connects Relution with the mesh. After the beacons have been enrolled, you can assign each beacon a message that the beacon will send out periodically. AT the moment Apple's iBeacons and Relution Tag messages are supported. The idea behind Relution Tag messages is that each beacon can send out a number of tags that can be mapped to actions, products or a specific behavior on a mobile device which constantly scans for these messages. The mobile device does not need internet access since the meaning of the tags is defined by the app itself. In contrast to iBeacons, however, it is possible to change the Relution tags inside a Relution Tag message and thus changing the behavior of the app. iBeacons, on the other side, are currently used for the realization of the conept of campaigns. Relution enables the user to define a campaign, a set of beacons can take part of. In each campaign one can define actions that will be triggered by the SDK on a mobile application, whenever the device receives an iBeacon message that is associated with an action in Relution.
